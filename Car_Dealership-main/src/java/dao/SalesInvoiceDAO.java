...
public class SalesInvoiceDAO {
    ...
    public int createInvoice(String invoicedate, String salesID, String carID, String custID) {
        int rs = 0;
        Connection cn = null;
        try {
            cn = DBUtils.getConnection();
            if (cn != null) {
                String sql = "INSERT INTO [dbo].[SalesInvoice]\n"
                        ...
                        + "           ,[carID]\n"
                        + "           ,[custID])\n"
                        + "     VALUES ((SELECT top 1 invoiceID from [dbo].[SalesInvoice] order by invoiceID DESC)+1 ,?,?,?,?)";
                PreparedStatement st = cn.prepareStatement(sql);
             
                st.setString(1, invoicedate);
                st.setString(2, salesID);
